...
function getPriceStringFormatted(price) {
  ...
  return `${new Intl.NumberFormat('fr-FR').format(parseInt(price))} DOLZ ($ ${priceInDollars.toFixed(2)})`;
}
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields({
      ...
      value:
        ...
        `Total DOLZ: ${new Intl.NumberFormat('fr-FR').format(dolzBalanceSeller)}\n` +
        `Total BabyDOLZ: ${new Intl.NumberFormat('fr-FR').format(babyDolzBalanceSeller)}\n`,
    });
  if (type === "sale") {
    // sale type
    ...
    embed.addFields({
      ...
      value:
        ...
        `Total DOLZ: ${new Intl.NumberFormat('fr-FR').format(dolzBalanceBuyer)}\n` +
        `Total BabyDOLZ: ${new Intl.NumberFormat('fr-FR').format(babyDolzBalanceBuyer)}\n`,
    });
  } else if (type === "offer") {
    // Listing type
